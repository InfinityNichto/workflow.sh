...
class reg_value_def_t(object):
    ...
    __repr__ = _swig_repr
    val: 'uint64' = property(_ida_regfinder.reg_value_def_t_val_get,
        _ida_regfinder.reg_value_def_t_val_set)
    """the value
"""
    def_ea: ida_idaapi.ea_t = property(_ida_regfinder.
        reg_value_def_t_def_ea_get, _ida_regfinder.reg_value_def_t_def_ea_set)
...
"""
    ...
    """as a SP delta
"""
    ABORTED = _ida_regfinder.reg_value_def_t_ABORTED
    """as an ABORTED value
"""
...
"""
    ...
    @staticmethod
    def make_aborted(bblk_ea: ida_idaapi.ea_t, aborting_depth: int=-1
        ) ->'reg_value_info_t':
...
"""
        return _ida_regfinder.reg_value_info_t_get_aborting_depth(self)
...
"""
    ...
    def set_aborted(self, bblk_ea: ida_idaapi.ea_t, aborting_depth: int=-1
        ) ->None:
...
:returns: NOT_COMPARABLE: values from R are added to THIS"""
    ...
    SAR = _ida_regfinder.reg_value_info_t_SAR
    ...
    def sar(self, r: 'reg_value_info_t', insn: 'insn_t const &') ->None:
        """Shift arithmetically the value right by R, save INSN as a defining instruction. 
        """
        return _ida_regfinder.reg_value_info_t_sar(self, r, insn)
...
def invalidate_regfinder_xrefs_cache(*args) ->None:
    """The data reference to TO has added (DREF!=dr_O) or removed (DREF==dr_O). Update the regtracker xrefs cache after this change. If TO == BADADDR then clear the entire xrefs cache. 
        """
    return _ida_regfinder.invalidate_regfinder_xrefs_cache(*args)
