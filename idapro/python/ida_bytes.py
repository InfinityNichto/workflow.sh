...
class compiled_binpat_vec_t(object):
    ...
    def _parse(self, ea: ida_idaapi.ea_t, text: str, radix: int,
        strlits_encoding: int) ->bool:
        return _ida_bytes.compiled_binpat_vec_t__parse(self, ea, text,
            radix, strlits_encoding)

    @staticmethod
    def parse(ea: ida_idaapi.ea_t, text: str, radix: int=-1,
        strlits_encoding: int=-1) ->compiled_binpat_vec_t:
...
"""get flags with FF_IVL & MS_VAL. but never use the debugger memory. 
        """
GFE_32BIT = _ida_bytes.GFE_32BIT
...
:returns: 0 if address is not present in the program"""
    return _ida_bytes.get_flags32(ea)
...
class octet_generator_t(object):
    ...
    ea: ida_idaapi.ea_t = property(_ida_bytes.octet_generator_t_ea_get,
        _ida_bytes.octet_generator_t_ea_set)
    avail_bits: int = property(_ida_bytes.octet_generator_t_avail_bits_get,
        _ida_bytes.octet_generator_t_avail_bits_set)
...
class data_type_t(object):
    ...
    """properties
"""
    name: str = property(_ida_bytes.data_type_t_name_get, _ida_bytes.
        data_type_t_name_set)
    """name of the data type. must be unique
"""
    menu_name: str = property(_ida_bytes.data_type_t_menu_name_get,
        _ida_bytes.data_type_t_menu_name_set)
    """Visible data type name to use in menus if nullptr, no menu item will be created 
        """
    hotkey: str = property(_ida_bytes.data_type_t_hotkey_get, _ida_bytes.
        data_type_t_hotkey_set)
    """Hotkey for the corresponding menu item if nullptr, no hotkey will be associated with the menu item 
        """
    asm_keyword: str = property(_ida_bytes.data_type_t_asm_keyword_get,
        _ida_bytes.data_type_t_asm_keyword_set)
...
"""
    name: str = property(_ida_bytes.data_format_t_name_get, _ida_bytes.
        data_format_t_name_set)
    """Format name, must be unique.
"""
    menu_name: str = property(_ida_bytes.data_format_t_menu_name_get,
        _ida_bytes.data_format_t_menu_name_set)
    """Visible format name to use in menus if nullptr, no menu item will be created 
        """
    hotkey: str = property(_ida_bytes.data_format_t_hotkey_get, _ida_bytes.
    ...
    """size of the value in bytes 0 means any size is ok data formats that are registered for standard types (dtid 0) may be called with any value_size (instruction operands only) 
        """
    text_width: int = property(_ida_bytes.data_format_t_text_width_get,
