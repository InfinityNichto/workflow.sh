...
class fixup_data_t(object):
    ...
    """selector of the target segment. BADSEL means an absolute (zero based) target. 
        """
    off: ida_idaapi.ea_t = property(_ida_fixup.fixup_data_t_off_get,
        _ida_fixup.fixup_data_t_off_set)
    """target offset 
        """
...
class fixup_info_t(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v),
        doc='The membership flag')
    __repr__ = _swig_repr
    ea: ida_idaapi.ea_t = property(_ida_fixup.fixup_info_t_ea_get,
        _ida_fixup.fixup_info_t_ea_set)
